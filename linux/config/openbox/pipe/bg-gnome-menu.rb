...
class Bgdir
  ...
  def print
    ...
    @menu.each do |item|
      ...
      if $*.to_s =~ /gnome/
        puts " <action name=\"execute\"><execute>gconftool-2 -s /desktop/gnome/background/picture_filename -t string \"#{@dirname}/#{item.filename}\"</execute></action>"
        puts " <action name=\"execute\"><execute>gconftool-2 -s /desktop/gnome/background/picture_options -t string #{@mode}</execute></action>"
      else
